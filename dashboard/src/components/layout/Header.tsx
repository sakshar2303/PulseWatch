  ...
  return (
    <header className="h-14 border-b border-white/[0.07] bg-void/90 backdrop-blur-xl px-5 flex items-center justify-between sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        {/* Breadcrumb Path */}
        <div className="hidden md:flex items-center gap-1.5 text-xs font-mono text-pulse-tertiary pl-4 border-l border-white/[0.08]">
          <span className="text-pulse-secondary hover:text-white cursor-pointer transition-colors">prod-cluster</span>
          <span>/</span>
          <span className="text-pulse-coral font-medium">ground-truth-verifier</span>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-2 px-2.5 py-1 rounded-full bg-void-card border border-white/[0.08] text-xs font-mono">
          <span className={`w-1.5 h-1.5 rounded-full ${allHealthy ? 'bg-pulse-emerald animate-pulse' : 'bg-pulse-coral animate-ping'}`} />
      ...
      <div className="flex items-center gap-2.5">
        ...
        {onOpenCommandPalette && (
          <button
            ...
            className="hidden md:flex items-center gap-2 px-2.5 py-1 rounded-lg bg-void-card hover:bg-void-elevated border border-white/10 text-xs font-mono text-pulse-secondary hover:text-white transition-colors"
          >
            <Search className="w-3.5 h-3.5 text-pulse-coral" />
            <span>Search</span>
            <kbd className="text-[10px] bg-white/5 px-1 py-0.2 rounded border border-white/10 text-pulse-tertiary">⌘K</kbd>
          </button>
        )}
        ...
        <a
          ...
          className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-void-card hover:bg-void-elevated border border-white/10 hover:border-pulse-sky/40 text-xs font-mono text-pulse-sky transition-colors"
        >
          <span>Jaeger UI</span>
          <ExternalLink className="w-3 h-3 text-pulse-sky" />
        </a>
          ...
          {(Object.keys(TIME_RANGE_CONFIGS) as TimeRangePreset[]).map((preset) => (
            <button
              ...
              className={`px-2 py-1 text-xs font-mono rounded-md transition-all ${
                timeRange === preset
                  ? 'bg-pulse-coral/20 text-pulse-coral font-semibold border border-pulse-coral/30 shadow-sm'
        ...
        <div className="hidden sm:flex items-center bg-void-card rounded-lg border border-white/[0.08] px-2 py-1 text-xs font-mono text-pulse-secondary">
          <span className="text-pulse-tertiary mr-1.5">Sync:</span>
          <select
        ...
        <button
          ...
          className="flex items-center justify-center w-7 h-7 rounded-lg bg-void-card border border-white/[0.08] hover:border-pulse-coral/40 hover:text-pulse-coral text-pulse-secondary transition-colors"
        >
          <RefreshCw className={`w-3.5 h-3.5 ${isRefreshing ? 'animate-spin text-pulse-coral' : ''}`} />
        </button>
      </div>
    </header>
  );
